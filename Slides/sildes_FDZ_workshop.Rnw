...
\begin{itemize}
  ...
  \item RStudio-Installation (optional) \\
  $\rightarrow$ Alternativen: Gitkraken, SmartGit, ...
...
\begin{itemize}
  \item Erstellen eines Online-Repositorys (z.B. auf Github)
  \begin{itemize}
    \item Benutzen einer R-spezifischen .gitignore
    \item Initialisieren mit einer kurzen Readme-Datei (.md)
  \end{itemize}
  \item Repository lokal auf den PC clonen
  \item (optional) R-Projekt in bestehendes Repository einf{\"u}gen
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Welche Dateien sollen nicht archiviert werden? \\
...
\begin{itemize}
  \item Davor: Lokales Repo synchronisieren (\textbf{pullen})
